...
export function createStructuresAddEditor(entitiesID,name, callback) {
    //OPTIONS
    ...
    createDialogue({
        ...
        onConfirm:(result)=>{
            ...
            var structuresRepo = createStructuresManagement()
            // var payload={name:result.name, type:result.entitiesType[0].uuid, iconPath:result.entitiesType[0].iconPath}
            var payload={name:result.name, type:"folder"}
            structuresRepo.add(payload)
